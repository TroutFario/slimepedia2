            ...
            {tab === ItemType.Toy
                ? Object.values(Toy).map((toy) => (
                    <NavLink key={toy + "-item"} to={"/items/toys/" + toy} style={{textDecoration: "none"}}>
                        <NavButton icon={"toys/" + toy} name={toyList[toy].name} size={1.25} selected={toy === item}/>
                    </NavLink>
                ))
                : Object.values(Resource).map((resource) => (
                    <NavLink key={resource + "-item"} to={`/items/${tab}/${resource}`} style={{textDecoration: "none"}}>
                        <NavButton
                            icon={tab + "/" + resource}
                            name={resourceList[resource].name}
                            size={1.25}
...
const ItemInfos: React.FC<ItemInfosProps> = ({item, category}) => {
    ...
    if (category === ItemType.Resource && Object.values(Resource).includes(item as Resource)) {
        ...
        title = resourceList[item as Resource].name;
        subtitle = resourceList[item as Resource].description;
        biomeList = resourceList[item as Resource].spawn;
        littleBoxList.pop();
        littleBoxList.push({
            image: "/assets/misc/pediatut.png",
            alt: "Pedia Informations Icon",
        ...
        littleBoxList.push(
            {
                image: "/assets/misc/pediatut.png",
                alt: "Pedia Informations Icon",
                subtitle: toyList[item as Toy].description,
            },
            {
                image: `/assets/misc/buck.png`,
                alt: "Newbuck Icon",
                title: "Price",
                subtitle: toyList[item as Toy].price.toString(),
                link: null,
            },
        );
        ...
        title = toyList[item as Toy].name;
...
const Items = () => {
    ...
    const title = (() =>
            category === ItemType.Resource
                ? resourceList[item as Resource].name
                : toyList[item as Toy].name.toString()
    )();
