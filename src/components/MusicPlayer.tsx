...
const playAudio = (
    ...
    if (audioRef.current) {
        ...
        audioRef.current
            .play()
            .then(() => setCurrentAudio(audioRef.current))
            .catch((err) => console.error("Impossible de lancer l'audio:", err));
    }
};
...
const setVideoTime = (videoRef: React.RefObject<HTMLVideoElement> | null, time: number, played: boolean) => {
    if (videoRef?.current && played) {
        ...
        videoRef.current.play().then(() => {
            if (videoRef.current !== null) videoRef.current.currentTime = time
        });
    }
};
